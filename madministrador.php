...
if($hacer=="eliminar"){
 $consulta = mysqli_query($conn,"DELETE FROM user_passwords WHERE `user_passwords`.`user` = '$id'");
 return;
}
 
  $consulta = mysqli_query($conn,"UPDATE `user_passwords` SET `passwordin` = '$password', `nombre` = '$name' ,  `addres` = '$direccion' ,  `tiemporesidencia` = '$Residencia' ,  `telefono` = '$Telefono' ,  `edad` = '$edad' ,  `puestosolicita` = '$solicita' ,  `salario` = '$Salario' ,  `horassemanalespuedetrabajar` = '$horassemanales' ,  `comenzaratrabajar` = '$Cuandodisponible'  WHERE `user_passwords`.`user` = '$id'");
  ...
  while ($respuesta = mysqli_fetch_assoc($consulta)) {
    ...
    <input id="busqueda" name="porfin" type="text" size="70" placeholder="INGRESE EL USUARIO A BUSCAR AQUI" value="'.$usuarioregres.'">
    </input>
    <input name="passwordin" type="submit" value="CONSULTAR" class="enviar">
      ...
      $part4_1='<tr><td colspan="5">usuario:<input name="user" type="text" size="30" placeholder="INGRESE UN USUARIO" value="'.$usuarioregres.'" ></input>contrasena:<input id="pass" name="password" type="text" size="30" placeholder="INGRESE UNA CONTRASENA" value="'.$passregres.'" required></input></td></tr>';
      $part5='<tr><td colspan="5">Nombre:<input id="nombre" name="name" type="text" size="70" placeholder="INGRESE SU NOMBRE AQUI" value="'.$nombre.'" style="text-transform:uppercase;" onchange="javascript:this.value=this.value.toUpperCase();" required></input></td></tr>';
      ...
      $part8='<tr><td colspan="5">Tiempo de Residencia:<input id="Residencia" name="Residencia" type="text" size="60" placeholder="INGRESE SU TIEMPO DE RESIDENCIA AQUI" value="'.$tiemporesidencia.'" maxlength="2" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" required></input></td></tr>';
      ...
      $part12='<tr><td colspan="5">Salario deseado:  $<input id="Salario" name="Salario" type="text" size="35" placeholder="INGRESE EL Salario QUE SOLICITA AQUI" value="'.$salario.'" maxlength="5" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" required title="SOLO PUEDE INGRESAR VALORES MENOS A 35000"></input></td></tr>';
